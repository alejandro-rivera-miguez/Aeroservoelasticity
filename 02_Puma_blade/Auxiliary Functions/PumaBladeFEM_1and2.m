%% =======================================================================
% FUNCTION: PumaBladeFEM_1and2
% DESCRIPTION: Computes the aeroelastic stability of the Puma helicopter
%              blade in hover using a 2-mode basis (1st flap, 1st torsion).
%              Evaluates either quasi-steady or unsteady aerodynamics.
%
% AUTHORS: Alejandro Rivera Míguez
%          (Based on course material by G. Quaranta)
%
% INPUTS:
%   QuasiSteady - Boolean flag (true for quasi-steady, false for unsteady)
% OUTPUTS:
%   stability   - Boolean indicating if the system is stable
%   damping_box - Array containing damping values for the modes
%% =======================================================================
function [stability, damping_box] = PumaBladeFEM_1and2(QuasiSteady)
    ...
    % Parameter Interpolation over the grid
    ...
    [V_raw, E_val] = eig(-Kt, M); % "Raw" eigenvectors (MATLAB normalizes them to euclidean norm 1, not mass)
    [Eo, I_sort] = sort(diag(E_val), 'descend'); 
    
    % Keep only 2 modes (Task 1 & 2 requirement)
    idx = [1, 2];
    f = imag(sqrt(Eo(idx)));
    No = length(idx);
    ...
    if f > 1e-6
        ...
        % M_gen_check = V_sel.' * M * V_sel; % This should yield Identity
        % 
        % fprintf('\n=== MASS ORTHOGONALITY VERIFICATION ===\n');
        ...
        %% Aerodynamic Integrals computation
        ...
        %% 4. AEROELASTIC SOLVER (p-k METHOD)
        tolerancia = 10e-6;
        ...
        for modes = 1:No
            while tol > tolerancia
                ...
                % Spanwise-varying reduced frequency
                ...
                % Numerical Integration across the span
                for i = 1:No
                    for j = 1:No
                ...
                k = 4/3 * eigenvalues(modes) * b / Omega / L;
                damping = -real(z_val) / eigenvalues(modes); % Corrected damping definition
                tol = diff1;
        ...
        %% 5. RESULTS OUTPUT
        sigma_1 = -damping_box(1) * freq_box(1);
        omega_d_1 = freq_box(1) * sqrt(1 - damping_box(1)^2);
        
        sigma_2 = -damping_box(2) * freq_box(2);
        omega_d_2 = freq_box(2) * sqrt(1 - damping_box(2)^2);
        
        if QuasiSteady 
            fprintf('\n=========================================\n');
            fprintf('     TASK 1 RESULTS (QUASISTEADY)\n');
            fprintf('=========================================\n');
        else
            fprintf('\n=========================================\n');
            fprintf('     TASK 2 RESULTS (UNSTEADY)\n');
            fprintf('=========================================\n');
        end
        
        fprintf('Mode 1 (Flap):    Lambda = %.2f +/- %.2fi [1/s] | Zeta = %.4f\n', sigma_1, omega_d_1, damping_box(1));
        fprintf('Mode 2 (Torsion): Lambda = %.2f +/- %.2fi [1/s] | Zeta = %.4f\n', sigma_2, omega_d_2, damping_box(2));
        fprintf('-----------------------------------------\n');
    else
        damping_box = zeros(1, No);
    end
