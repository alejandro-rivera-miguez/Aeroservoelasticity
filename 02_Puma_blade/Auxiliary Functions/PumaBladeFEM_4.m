%% =======================================================================
% FUNCTION: PumaBladeFEM_4
% DESCRIPTION: Computes the aeroelastic stability of the Puma helicopter
%              blade in hover using a full structural basis (6 modes).
%              Evaluates unsteady aerodynamics with the reduced frequency 
%              calculated at a reference section (typically 75% span).
%
...
%% =======================================================================
function [stability, damping_box] = PumaBladeFEM_4(link_stiff, cg_off)
    ...
    [V_raw, E_val] = eig(-Kt, M); % "Raw" eigenvectors (MATLAB normalizes to euclidean norm 1, not mass)
    [Eo, I_sort] = sort(diag(E_val), 'descend'); 
    
    % Extract full basis (6 modes)
    idx = [1, 2, 3, 4, 5, 6];
    f = imag(sqrt(Eo(idx)));
    No = length(idx);
    ...
    if f > 1e-6
        ...
        % Final generalized structural matrices
        M = eye(No);                 
        K = diag(f.^2);              
        ...
        for modes = 1:No
            while tol > tolerancia
                % Calculate reduced frequency using a reference section (typically 75% R)
                ...
                % Generalized Aero Matrices Assembly (Based on modal reduction logic)
        ...
        % (Optional console output can go here)
